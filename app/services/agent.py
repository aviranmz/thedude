...
async def handle_user_request(data):
    try:
        ...
        if not isinstance(dates, dict):
            logging.warning(f"Invalid dates: {dates}")
            dates = {}
        ...
        logging.info(f"Hotels: {hotels}")
        logging.info(f"Trip Info: {trip_info}")
        ...
        logging.exception("Fatal error in /agent handler")

        # ✅ safe fallback MCP-compliant response
        return {
            "reply": "The service was not able to process your request. Please try again.",
            "can_search": False,
            "search_types": [],
            "missing_fields": [],
            "flights": [],
